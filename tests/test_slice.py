...
def test_integer_transform(transformer):
    ...
    # Negative index
    assert transformer.transform(-1) == (
        slice(-1, 0), slice(None), slice(None), slice(None)
    )
...
def test_slice_transform(transformer):
    ...
    assert transformer.transform(slice(1, 10, 2)) == (
        slice(1, 10, 2), slice(None), slice(None), slice(None)
    )
    ...
    assert transformer.transform(slice(-3, -1)) == (
        slice(-3, -1), slice(None), slice(None), slice(None)
    )

# Advanced Transformation Tests
# ---------------------------

@pytest.mark.parametrize("lst,dim,expected", [
    ([1, 3], 0, (
        (slice(1, 2), slice(3, 4)), 
...
def test_list_with_dimension(transformer, lst: List[int], dim: int, 
                           expected: Tuple[Any, ...]):
...
def test_tuple_transform(transformer):
    ...
    assert transformer.transform((1, slice(1, 3), 2)) == (
        slice(1, 2), slice(1, 3), slice(2, 3), slice(None)
    )
    
    # Full specification
    assert transformer.transform((1, 2, 3, 4)) == (
        slice(1, 2), slice(2, 3), slice(3, 4), slice(4, 5)
    )
...
def test_ellipsis_handling(transformer):
    ...
    assert transformer.transform((1, Ellipsis, 2)) == (
        slice(1, 2), slice(None), slice(None), slice(2, 3)
    )
    
    # Ellipsis at start
    assert transformer.transform((Ellipsis, 1, 2)) == (
        slice(None), slice(None), slice(1, 2), slice(2, 3)
    )
